...
namespace sylar {
    ...
    Scheduler::Scheduler(size_t threads, bool use_caller, const std::string& name)
        ...
        if(use_caller){
            ...
            sylar::Thread::SetName(m_name); //防止线程名称没改
   ...
   //power:核心函数，调度器停止
    void Scheduler::stop(){
        ...
        */
        ...
        //有多个线程（是一组线程池），先设置标志位m_stopping，唤醒tick0le()其他子线程根据该标志位退出；都完毕之后，再将调度协程唤醒退出

        //主线程Id不为-1说明是创建调度器(use_caller)的线程
    ...
    */
    void Scheduler::run(){
        ...
        setThis();
        //当前线程ID不等于主线程ID
        if(sylar::GetThreadId() != m_rootThread){//当前线程未作为调度线程使用？？？
            t_scheduler_fiber = Fiber::GetThis().get();
        }

        Fiber::ptr idle_fiber(new Fiber(std::bind(&Scheduler::idle, this)));//power:创建一个专门跑idel()的协程，调度任务都完成之后去做idle
        ...
        1. 加锁，取出可执行对象容器/消息队列/任务队列m_fibers中的元素，解锁
            a. 如果当前的可执行对象没有指定线程 且不是 当前在跑的线程要执行的，就跳过。并且设置一个信号量，如bool is_tickle以便通知其他线程来执行这个属于它们的可执行对象（任务）
            b. 如果当前的可执行对象是当前在跑的线程要执行的，检查协程体和回调函数是否为空，为空断言报错；不为空取出，从队列删除该元素
            c. 如果没有一个可执行对象是当前跑的线程应该执行的，就设置一个标志bool is_work表明当前是否有任务可做，没有转去执行idle()函数

        2. 开始执行从队列中拿出的可执行对象，分为：协程和函数。（本质都是要调度协程，只是兼容传入的可执行对象是函数的情况）
            a. 如果为协程，并且当前线程应该工作is_work = true，分情况讨论：
                ⅰ. 协程处于没有执行完毕TERM且没有异常EXCEPTION，就swapIn()调入（继续）执行。调回（不一定是执行完毕了，也有可能到时了）后，如果处于就绪状态READY需要再一次加入队列中；
        ...
        while(true){
            ...
            //加锁
            {
                ...
                while(it != m_fibers.end()){
                    //a.当前任务没有指定线程执行 且 不是我当前线程要处理的协程 跳过，不需要处理
