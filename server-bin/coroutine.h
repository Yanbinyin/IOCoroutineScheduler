...
/*
    ...
    基本原理：采用非对称协程开发。所有子协程的调度和创建依赖于一个母协程init_coroutine，
        ...
        通过母协程让出当前代码序列在CPU的执行权，转让到子协程对应的代码序列中继续执行，子协程完毕后让出执行权又回到母协程中。
*/
...
/*//add: 知识储备
    ...
    ● int setcontext(const ucon    text_t *ucp); 将保存的程序上下文拿出推送到CPU上
    ...
    注意：使用makecontext时，必须先调用一次getcontext（个人猜测是因为，重新创建一个上下文使用除了要传函数的入口地址，其余的CPU寄存器值也要保存，getcontext可以代劳），
...
/*//add:
    ...
    p.reset(q) //q为智能指针指向的对象，这样会使智能指针p指向q的空间，而且释放原来的空间(默认是delete)
*/
/*power:
...
namespace sylar {
    ...
    public:
        //cb:协程执行的函数  stacksize:协程栈大小  use_caller:是否在MainFiber上调度
        Fiber(std::function<void()> cb, size_t stacksize = 0, bool use_caller = false);
        ~Fiber();
        
        //重置协程执行函数,并设置状态，getState() = INIT。协程执行完了，但是分配的内存没释放，基于这段内存创建新的协程
        void reset(std::function<void()> cb);
        void swapIn();  //将当前协程切换到运行状态，getState() = EXEC
        void swapOut(); //将当前协程切换到后台
        void call();    //将当前线程切换到执行状态，执行的为当前线程的主协程
        void back();    //将当前线程切换到后台 pre:执行的为该协程 post:返回到线程的主协程
