...
//hack: 结构体的作用？？？ 重载 << >>？？？
    ...
    int IOManager::addEvent(int fd, Event event, std::function<void()> cb){
        ...
        ev.events = EPOLLET | fd_ctx->events | event; //EPOLLET:位掩码//EPOLLET + 原来event + 当前的
        ev.data.ptr = fd_ctx; //回调的时候，通过数据字段(data)拿回在哪个fd_ctx上面触发的
    ...
    bool IOManager::cancelEvent(int fd, Event event){
        ...
        fd_ctx->triggerEvent(event);//主动触发句柄上事件绑定的回调函数 重新加入到任务队列里
        return true;
    }
    ...
    */
    ...
    while(1){
        ...
        4. 如果有就绪IO，就带回对应IO：
            ...
            b. 管道读IO，由tickle()函数发送消息导致的IO活动，用于唤醒，将其忽略（不绑定回调函数作为任务去调度仅仅是唤醒作用）
    ...
    */
    void IOManager::idle(){
        ...
        std::shared_ptr<epoll_event> shared_events(evts, [](epoll_event* ptr){ delete[] ptr; });
        ...
        while(true){
            ...
            for(int i = 0; i < rt; ++i){
                ...
                int left_events = (fd_ctx->events & ~real_events);  //剩余事件：当前事件 - 触发事件
                int op = left_events ? EPOLL_CTL_MOD : EPOLL_CTL_DEL; //有事件：修改 无事件：删除
                ev.events = EPOLLET | left_events; //复用event，ET模式 + 剩余事件后添加到epoll中
    ...
    //基类stopping() + 多判断一下待处理事件数量m_pendingEventCount和定时器队列是否空isTimersEmpty()
