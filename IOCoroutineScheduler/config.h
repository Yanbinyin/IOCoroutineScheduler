...
private:
  ...
  std::map<uint64_t, on_change_cb>
      m_cbs; // 配置项所绑定的回调函数容器 uint64_t key要求唯一，一般可以用哈希
...
public:
  ...
  static typename ConfigVar<T>::ptr Lookup(const std::string &name) {
    ...
    return std::dynamic_pointer_cast<ConfigVar<T>>(
        it->second); // power:与dynamic_cast的区别
  }
  // 重载
  template <class T>
  ...
  LookupBase(const std::string &name); // 返回配置项的基类指针
private:
  // power:static变量未规定初始化顺序，Lookup拿变量的时候可能发现s_datas还未初始化
  static ConfigVarMap &GetDatas() {
    static ConfigVarMap
        s_datas; // map存储所有的配置项的<name, value>部分，<std::string,
                 // ConfigVarBase::ptr>类型
    return s_datas;
  }
};
