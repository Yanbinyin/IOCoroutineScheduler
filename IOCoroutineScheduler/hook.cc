...
namespace bin{
    ...
    static bin::ConfigVar<int>::ptr g_tcp_connect_timeout = bin::Config::Lookup("tcp.connect.timeout", 5000, "tcp connect timeout");
...
extern "C"{
    ...
    unsigned int sleep(unsigned int seconds){
        ...
        bin::IOManager* iom = bin::IOManager::GetThis();
        //power: 指针//inline void bin::Scheduler::schedule<bin::Fiber::ptr>(bin::Fiber::ptr fc, int thread)
        //iom->addTimer(usec/1000, iom->schedule(fiber, -1));
        iom->addTimer(seconds * 1000, std::bind((void(bin::Scheduler::*)
                    (bin::Fiber::ptr, int thread))&bin::IOManager::schedule
                    ,iom, fiber, -1)); 
        bin::Fiber::YieldToHold();
        return 0;
    }
    ...
    int usleep(useconds_t usec){
        ...
        bin::Fiber::ptr fiber = bin::Fiber::GetThis();
        bin::IOManager* iom = bin::IOManager::GetThis();
        iom->addTimer(usec / 1000, std::bind((void(bin::Scheduler::*)
                (bin::Fiber::ptr, int thread))&bin::IOManager::schedule
                ,iom, fiber, -1));
        bin::Fiber::YieldToHold();
        return 0;
    ...
    int nanosleep(const struct timespec *req, struct timespec *rem){
        ...
        bin::Fiber::ptr fiber = bin::Fiber::GetThis();
        bin::IOManager* iom = bin::IOManager::GetThis();
        iom->addTimer(timeout_ms, std::bind((void(bin::Scheduler::*)
                (bin::Fiber::ptr, int thread))&bin::IOManager::schedule
                ,iom, fiber, -1));
        bin::Fiber::YieldToHold();
        return 0;
    ...
    */
    ...
    int accept(int s, struct sockaddr *addr, socklen_t *addrlen){
        int fd = do_io(s, accept_f, "accept", bin::IOManager::READ, SO_RCVTIMEO, addr, addrlen);
        if(fd >= 0)
            bin::FdMgr::GetInstance()->get(fd, true); //把新建立的通信套接字加入到FdManager中去管理
        return fd;
    }

    ssize_t read(int fd, void *buf, size_t count){
        //do_io(int fd, OriginFun fun, const char* hook_fun_name, uint32_t event, int timeout_so, Args&&... args){
        return do_io(fd, read_f, "read", bin::IOManager::READ, SO_RCVTIMEO, buf, count);
    }

    ssize_t readv(int fd, const struct iovec *iov, int iovcnt){
        return do_io(fd, readv_f, "readv", bin::IOManager::READ, SO_RCVTIMEO, iov, iovcnt);
    }

    ssize_t recv(int sockfd, void *buf, size_t len, int flags){
        return do_io(sockfd, recv_f, "recv", bin::IOManager::READ, SO_RCVTIMEO, buf, len, flags);
    }

    ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen){
        return do_io(sockfd, recvfrom_f, "recvfrom", bin::IOManager::READ, SO_RCVTIMEO, buf, len, flags, src_addr, addrlen);
    }

    ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags){
        return do_io(sockfd, recvmsg_f, "recvmsg", bin::IOManager::READ, SO_RCVTIMEO, msg, flags);
    }

    ssize_t write(int fd, const void *buf, size_t count){
        return do_io(fd, write_f, "write", bin::IOManager::WRITE, SO_SNDTIMEO, buf, count);
    }

    ssize_t writev(int fd, const struct iovec *iov, int iovcnt){
        return do_io(fd, writev_f, "writev", bin::IOManager::WRITE, SO_SNDTIMEO, iov, iovcnt);
    }

    ssize_t send(int s, const void *msg, size_t len, int flags){
        return do_io(s, send_f, "send", bin::IOManager::WRITE, SO_SNDTIMEO, msg, len, flags);
    }

    ssize_t sendto(int s, const void *msg, size_t len, int flags, const struct sockaddr *to, socklen_t tolen){
        return do_io(s, sendto_f, "sendto", bin::IOManager::WRITE, SO_SNDTIMEO, msg, len, flags, to, tolen);
    }

    ssize_t sendmsg(int s, const struct msghdr *msg, int flags){
        return do_io(s, sendmsg_f, "sendmsg", bin::IOManager::WRITE, SO_SNDTIMEO, msg, flags);
    }
