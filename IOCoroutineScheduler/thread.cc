...
static thread_local Thread *t_thread = nullptr; // current thread pointer
static thread_local std::string t_thread_name = "UNKNOW"; // current thread name
static bin::Logger::ptr g_logger = BIN_LOG_NAME("system"); // log
...
Thread::Thread(std::function<void()> cb, const std::string &name)
  ...
  // 在调用pthread_create()线程API的时候，传入this指针，就能通过这个指针去访问到类中的成员，
  // 否则类成员静态方法中无法使用非静态成员变量。
  // 或者，另外写一个普通成员函数，在线程回调函数中去调用那个函数也能解决该问题.
...
void Thread::join() {
  ...
  // BIN_LOG_INFO(g_logger) << "name: " << m_name << " join";
}

void *Thread::run(void *arg) {
  // 必须为static func, void*，保证线程能够接受任意类型的参数，到时候再强制转换
