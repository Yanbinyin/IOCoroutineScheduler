...
namespace bin
{
    ...
    public:
        ...
        void format(std::ostream& os, Logger::ptr logger, LogLevel::Level level, LogEvent::ptr event) override{
            ...
            localtime_r(&time, &tm);//将参数time所指的数据转换成从公元1970年1月1日0时0分0 秒算起至今的UTC时间所经过的秒数，并存在结果体tm中
            char buf[64];            
            //c_str(): 返回当前字符串的首字符地址
    ...
    Logger::Logger(const std::string &name) 
        ...
        m_formatter.reset(new LogFormatter("%d{%Y-%m-%d %H:%M:%S}%T%f:%l%T[%p]%T%c%T%t(%N)%T%F%T%m%n")); //bin add
    ...
    void LogFormatter::init(){
        ...
        for(size_t i = 0; i < mLF_pattern.size(); ++i){
            ...
            */
            ...
            if((i + 1) < mLF_pattern.size() && mLF_pattern[i + 1] == '%'){//power:这两个条件的顺序不能更换，先判断数组是否越界，再判断数组内容
                nstr.append(1, '%');
                continue;
            }
            ...
            */
            ...
            while(n < mLF_pattern.size()){
                /*  break：
                        ...
                        字母和 { 不跳出，空格、特殊字符等都跳出
                */
                if(!fmt_status && (!isalpha(mLF_pattern[n]) && mLF_pattern[n] != '{' && mLF_pattern[n] != '}')){
            ...
            if(fmt_status == 0){
                if(!nstr.empty()){
                    ...
                    vec.push_back(std::make_tuple(nstr, std::string(""), 0));    //nstr 是符号[ ] :
                    nstr.clear();
                }
                ...
                //解析出现了问题：  fmt_status == 1 表明上面while遍历时，遇到 '{' 进入if赋值1，但是没有遇到 '}' 进入else if给它赋值为0
                //std::cout << "pattern parse error: " << mLF_pattern << " - " << mLF_pattern.substr(i) << std::endl;
                m_error = true;
                //vec.push_back 3：
    ...
    #undef XX
        ...
        for(auto& i : vec){
            if(std::get<2>(i) == 0){
                mLF_FI_items.push_back(FormatItem::ptr(new StringFormatItem(std::get<0>(i))));
            }else{
                auto it = s_format_items.find(std::get<0>(i));
                if(it == s_format_items.end()){
                    mLF_FI_items.push_back(FormatItem::ptr(new StringFormatItem("<<error_format %" + std::get<0>(i) + ">>")));
                    m_error = true;
                }else{
    ...
    public:
        LogDefine operator()(const std::string& v){
            ...
            //todo:LogDefine加上 file 属性，然后这里更改掉就可以更改配置的路径，但是目前有bug
    ...
    bin::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =  bin::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
        ...
        LogIniter(){
            ...
            //g_log_defines->addListener(0xF1E231, [](const std::set<LogDefine>& old_value, const std::set<LogDefine>& new_value){
                ...
                for(auto& i : new_value){
                    ...
                    //设置或修改日志
                    std::cout << "** " << i.name << " level=" << i.level << "  " << logger << std::endl;                    
