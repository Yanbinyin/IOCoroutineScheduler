...
bool Module::onConnect(bin::Stream::ptr stream){
    return true;
}

bool Module::onDisconnect(bin::Stream::ptr stream){
    return true;
}
    ...
    for(auto& i : svrs){
        ...
        for(auto& s : socks){
            ...
            if(str.find("0.0.0.0") == 0){
                ip_and_port = bin::GetIPv4() + ":" + std::to_string(addr->getPort());
            } else {
...
bool RockModule::handleRequest(bin::Message::ptr req
                               ,bin::Message::ptr rsp
                               ,bin::Stream::ptr stream){
    auto rock_req = std::dynamic_pointer_cast<bin::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<bin::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<bin::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(bin::Message::ptr notify
                              ,bin::Stream::ptr stream){
    auto rock_nty = std::dynamic_pointer_cast<bin::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<bin::RockStream>(stream);
...
void ModuleManager::init(){
    ...
    bin::FSUtil::ListAllFile(files, path, ".so");
