...
#include "bin/mutex.h"
#include "bin/util.h"
#include "bin/streams/service_discovery.h"
#include <vector>
...
public:
    ...
    uint32_t incUsedTime(uint32_t v){ return bin::Atomic::addFetch(m_usedTime ,v);}
    uint32_t incTotal(uint32_t v){ return bin::Atomic::addFetch(m_total, v);}
    uint32_t incDoing(uint32_t v){ return bin::Atomic::addFetch(m_doing, v);}
    uint32_t incTimeouts(uint32_t v){ return bin::Atomic::addFetch(m_timeouts, v);}
    uint32_t incOks(uint32_t v){ return bin::Atomic::addFetch(m_oks, v);}
    uint32_t incErrs(uint32_t v){ return bin::Atomic::addFetch(m_errs, v);}

    uint32_t decDoing(uint32_t v){ return bin::Atomic::subFetch(m_doing, v);}
    void clear();
...
public:
    typedef bin::RWMutex RWMutexType;
