...
void AsyncSocketStream::Ctx::doRsp(){
    ...
    if(!bin::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler*)nullptr)){
        return;
    }
    if(!scd || !fiber){
        return;
...
bool AsyncSocketStream::start(){
    if(!m_iomanager){
        m_iomanager = bin::IOManager::GetThis();
    }
    if(!m_worker){
        m_worker = bin::IOManager::GetThis();
    }
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get(){
    ...
    for(uint32_t i = 0; i < m_size; ++i){
        auto idx = bin::Atomic::addFetch(m_idx, 1);
