...
void LoadBalanceItem::close(){
    if(m_stream){
        ...
        bin::WorkerMgr::GetInstance()->schedule("service_io", [stream](){
            stream->close();
        });
    }
}
...
std::string LoadBalance::statusString(const std::string& prefix){
    ...
    lock.unlock();
    std::stringstream ss;
    ss << prefix << "init_time: " << bin::Time2Str(m_lastInitTime / 1000) << std::endl;
    for(auto& i : datas){
...
void SDLoadBalance::onServiceChange(const std::string& domain, const std::string& service
    ...
    BIN_LOG_INFO(g_logger) << "onServiceChange domain=" << domain
                             << " service=" << service;
    auto type = getType(domain, service);
    ...
    for(auto& i : add_values){
        ...
        if(!stream){
            BIN_LOG_ERROR(g_logger) << "create stream fail, " << i.second->toString();
            continue;
        }
