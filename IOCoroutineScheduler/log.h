...
#define BIN_LOG_ROOT()                                                         \
  bin::LoggerMgr::GetInstance()                                                \
      ->getRoot() // 返回一个唯一的singleton LogManager指针
...
private:
  ...
  std::vector<FormatItem::ptr>
      mLF_FI_items; // 解析后日志格式 具体格式输出的具体子类队列
...
public:
  ...
  // virtual void log(Logger::ptr logger, LogLevel::Level level,LogEvent::ptr
  // event) = 0;
...
private:
  ...
  LogFormatter::ptr m_formatter; // 日志格式器，自带的一个LogFormatter
                                 // 防止LogAppender没有LogFormatter
...
private:
  MutexType m_mutex;
  std::map<std::string, Logger::ptr>
      m_loggers; // 日志器容器  建立名字和日志器的映射关系
